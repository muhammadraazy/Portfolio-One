  ...
  return (
    <Section id="about">
          ...
          <SectionText> 
                   ...
                   my purpose in web development is to be a better and good developer, i'd like to spread a good vibe through the web technologies to the world and another developers out there </SectionText>
          <CarouselContainer ref={carouselRef} scroll={handleScroll} >
                <>
                    { TimeLineData.map(({year, text}, index) => (
                        <CarouselMobileScrollNode key={index} final={index === TOTAL_CAROUSEL_COUNT - 1 } >
                              <CarouselItem 
                                index={index} id={`carousel_item-${index}`}
                                active={activeItem}
